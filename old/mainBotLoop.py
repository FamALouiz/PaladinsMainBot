...
import os, sys, time, pyautogui, keyboard, win32gui, win32con, random, datetime, asyncio, threading, winreg, win32process, psutil
...
class MainBotLoop:
    ...
    def checkJumpSecs(self):
        try:
            a = float(self.jumpSecs[0])
            b = float(self.jumpSecs[1])
            if a > b or a < 0 or b < 0:
                raise ValueError("yikes")
            else:
                self.jumpSecs = (a, b)
        except ValueError:
            self.print_to_GUI("Invalid bus jump delay.", "error")
            return False
        else:
            return True
            ...
            if not self.checkJumpSecs():
                ...
                if self.fortniteHandle is not None:
                    if self.env != "development":
                        win32gui.ShowWindow(
                            self.fortniteHandle, win32con.SW_SHOWDEFAULT
                        )
