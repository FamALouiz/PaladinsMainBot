...
import mainBotLoop, asyncio, threading, time, uuid, winreg, datetime, os, sys, OpenSSL, ssl
...
class BotGUI:
    ...
    def initRegEntries(self):
        try:
            ...
            value, _ = winreg.QueryValueEx(key, "min_sec_jump")
            value, _ = winreg.QueryValueEx(key, "max_sec_jump")
    ...
    def resetRegEntries(self):
        try:
            ...
            winreg.SetValueEx(registry_key, "min_sec_jump", 0, winreg.REG_SZ, str(0))
            winreg.SetValueEx(registry_key, "max_sec_jump", 0, winreg.REG_SZ, str(25))
    ...
    def initMainFrame(self, tier):
        ...
        lblBusJump1 = Label(busJumpFrame, text="Jump from bus randomly")
        lblBusJump2 = Label(busJumpFrame, text="from: ")
        self.minSecJumpEntry = Entry(busJumpFrame, width=3)
        value, _ = winreg.QueryValueEx(key, "min_sec_jump")
        self.minSecJumpEntry.insert(0, value)
        lblBusJump3 = Label(busJumpFrame, text=" to: ")
        self.maxSecJumpEntry = Entry(busJumpFrame, width=3)
        value, _ = winreg.QueryValueEx(key, "max_sec_jump")
        self.maxSecJumpEntry.insert(0, value)
        lblBusJump4 = Label(busJumpFrame, text=" seconds")
        self.treesInt = IntVar()
        treeschkbtn = Checkbutton(
            optionsFrame,
            text="Land on top of trees (1920x1080 resolution only)",
            variable=(self.treesInt),
        )
        value, _ = winreg.QueryValueEx(key, "land_on_trees")
        if value == "on":
            treeschkbtn.select()
        ...
        if tier >= 1:
            ...
            lblBusJump2.grid(row=1, column=0, sticky=E)
            self.minSecJumpEntry.grid(row=1, column=1)
            lblBusJump3.grid(row=1, column=2)
            self.maxSecJumpEntry.grid(row=1, column=3)
            lblBusJump4.grid(row=1, column=4, sticky=W)
        ...
        if tier >= 2:
            treeschkbtn.grid(row=1, sticky=W)
        winreg.CloseKey(key)
    ...
    def startBot(self):
        ...
        minJumpSec = self.minSecJumpEntry.get()
        maxJumpSec = self.maxSecJumpEntry.get()
        ...
        winreg.SetValueEx(registry_key, "min_sec_jump", 0, winreg.REG_SZ, minJumpSec)
        winreg.SetValueEx(registry_key, "max_sec_jump", 0, winreg.REG_SZ, maxJumpSec)
        winreg.SetValueEx(
            registry_key,
            "land_on_trees",
            0,
